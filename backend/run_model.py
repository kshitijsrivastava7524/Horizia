...
def main(stack_path=None, site=None, date=None):
    ...
    # ---------- Validate inputs ----------
    if site is None or date is None:
        raise ValueError("site and date must be provided")

    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    date_str = date.strftime("%Y-%m-%d")

    out_dir = os.path.join(BASE_DIR, f"../data/output/{site}/{date_str}")
    os.makedirs(out_dir, exist_ok=True)

    OUT_PATH = os.path.join(out_dir, "prob.tif")
    BIN_PATH = os.path.join(out_dir, "binary.tif")
