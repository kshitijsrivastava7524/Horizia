...
def fetch_data_fun(site, date):
    ...
    end   = (date + timedelta(days=1)).strftime('%Y-%m-%d')  # inclusive of target date
    ...
    if s2_count == 0:
        print(f"[SKIP] No Sentinel-2 images for {site} between {start} and {end}")
        return None
    ...
    if s1_count == 0:
        print(f"[SKIP] No Sentinel-1 images for {site} between {start} and {end}")
        return None
    ...
    # Download
    print(f"[DOWNLOAD] {site} {date.date()}")
