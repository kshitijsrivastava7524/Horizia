...
from model_train_unet_4 import UNet   # Ensure file renamed (no numeric prefix)
import pyproj

# ---------- CONFIG ----------
STACK_PATH = "../data/processed/stack_normalized.tif"
MODEL_PATH = "../../models/horizia_unet_best.pth"
OUT_PATH   = "../data/output/predicted_lake_prob.tif"
PATCH      = 256
THRESH     = 0.5
...
# ---------- Binary mask ----------
BIN_PATH = OUT_PATH.replace("_prob.tif", "_binary.tif")
...
def pixel_area_m2(profile):
    """Compute pixel area in m², handling both degree & meter CRSs."""
    transform = profile["transform"]
    crs = profile["crs"]
    if "EPSG:4326" in str(crs):
        # Approx conversion from degrees to meters
...
print(f"Lake coverage: {(lake_pixels/(pred_mask.size))*100:.2f}%")
print("-----------------------")
